...
/**
 * This is a Javassist program which produce a new class representing vectors of
 ...
 *
 * <p>
 * requests the Javassist preprocessor to substitute the following lines for the
 ...
 *
 * <p>
 * The Javassist preprocessor calls <code>VectorAssistant.assist()</code> and
 * produces class <code>intVector</code> equivalent to:
 *
 * <ul>
 * 
 * <pre>
 ...
 *
 * <p>
 * <code>VectorAssistant.assist()</code> uses <code>sample.vector.Sample</code>
 * and <code>sample.vector.Sample2</code> as a template to produce the methods
 * <code>add()</code> and <code>at()</code>.
 */
public class VectorAssistant implements Assistant {
	public final String packageName = "sample.vector.";

	/**
	 * Calls <code>makeSubclass()</code> and produces a new vector class. This
	 * method is called by a <code>sample.preproc.Compiler</code>.
	 *
	 * @see sample.preproc.Compiler
	 */
	public CtClass[] assist(ClassPool pool, String vec, String[] args) throws CannotCompileException {
		if (args.length != 1)
	...
	/**
	 * Produces a new vector class. This method does not work if the element
	...
	/**
	 * Produces a new vector class. This uses wrapped methods so that the
	 * element type can be a primitive type.
	 *
	 * @param type
	 *            the type of elements
	 */
	public CtClass makeSubclass2(ClassPool pool, CtClass type)
		...
		CtMethod m = CtNewMethod.wrapped(CtClass.voidType, "add", args1, null, addmethod, null, vec);
		vec.addMethod(m);
		m = CtNewMethod.wrapped(type, "at", args2, null, atmethod, null, vec);
		vec.addMethod(m);
		vec.writeFile();
		return vec;
	}

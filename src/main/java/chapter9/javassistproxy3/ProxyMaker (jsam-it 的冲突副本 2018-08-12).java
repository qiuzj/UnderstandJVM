	...
	public static Object newProxyInstance(ClassLoader loader, Class<?>[] interfaces, InvocationHandler h) throws Throwable {
		...
		for (Class<?> interfaceClass : interfaces) {
			...
			for (int i = 0; i < ctMethods.length; i++) {
				// 4.2.1 为代理类添加反射方法字段, 如private Method m0;
				String methodFieldName = "m" + i; // 新的方法名
				String classStr = "new Class[0]";
				if (ctMethods[i].getParameterTypes().length > 0) {
					for (CtClass clazz : ctMethods[i].getParameterTypes()) {
						classStr = ((classStr == "new Class[0]") ? clazz.getName() : classStr + "," + clazz.getName()) + ".class";
					}
				...
				String methodFieldBody = String.format(methodFieldTpl, "m" + i, interfaceClass.getName(), ctMethods[i].getName(), classStr);

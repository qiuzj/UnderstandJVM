	...
	public static Object newProxyInstance(ClassLoader loader, Class<?>[] interfaces, InvocationHandler h) throws Throwable {
		...
		for (Class<?> interfaceClass : interfaces) {
			...
			for (int i = 0; i < ctMethods.length; i++) {
				...
				methodField.setModifiers(AccessFlag.PRIVATE);
				proxyCc.addField(methodField);
				
				/* 4.3.2 为方法添加方法体 */
				String methodBody = "$0.h.invoke($0, Class.forName(\"" + interfaceClass.getName() + "\").getDeclaredMethod(\"" + ctMethods[i].getName() + "\", $sig), $args)";
